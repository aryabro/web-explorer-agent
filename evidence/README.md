...
| [`qualification-fresh-session`](qualification-fresh-session/) | `success` | The newly compiled draft replayed in a fresh browser session before publication | 2026-09-19 00:08:23 |
...
## Scripted fixture evidence

[`fixture/`](fixture/) is generated by `scripts/generate_fixture_artifacts.py` using the deterministic `ScriptedModel`. It is useful for development and schema regression, but it is **not** the assignment's genuine model-run evidence. The genuine run is `discovery-live-read-savings`.

The fixture namespace is intentionally left as `evidence/fixture/` because the generator and fixture capability provenance use that stable path.
